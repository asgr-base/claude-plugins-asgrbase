...
```bash
# リポジトリをクローン
git clone https://github.com/asgr-base/claude-plugins-asgrbase.git

# 必要なスキルを ~/.claude/skills/ にコピー
cp -r claude-plugins-asgrbase/plugins/jp-law-verification ~/.claude/skills/
```
...
| [mf-cli](plugins/mf-cli/) | MoneyForward Cloud Accounting をコマンドラインから操作 | 1.4.0 |
...
| [jp-kobunsho-xml-to-pdf](plugins/jp-kobunsho-xml-to-pdf/) | e-Gov 公文書 ZIP（XML+XSL）を 1 ページに収まる PDF / Markdown に変換（v2.1.0 で `--format pdf/md/both` 対応） | 2.1.0 |
...
- [Anthropic Claude Code プラグイン](https://github.com/anthropics/claude-code/tree/main/plugins)
