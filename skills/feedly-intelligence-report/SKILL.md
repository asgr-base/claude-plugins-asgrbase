...
1. **トークンリフレッシュ**
   - タイムアウトは**300秒**を使用（120秒ではタイムアウトする）
   - リフレッシュ失敗時はエラーログを確認し、手動対応を案内
...
6. **Recommendation選定（必須）**
   - スクリプトがレポートに「Recommendation候補 (AI選定待ち)」セクションを自動生成
   - 候補10件からeKYC・AI・テック視点で注目に値する5件程度を選定
   - 各記事に選定理由（1行）を記載
   - セクション見出しを `## Recommendation (N件)` に書き換え
   - 候補テーブルを選定結果テーブル（記事・選定理由・読了・保存）に置換
...
### 5. Recommendation選定（Claude Code実行・必須）

スクリプトがレポート内に「## Recommendation候補 (AI選定待ち)」セクションを自動生成する。
Claude Codeはこの候補から5件程度を選定し、レポートを完成させる。

**IMPORTANT**: このステップを実行しないとレポートは未完成状態（「AI選定待ち」が残る）。

**選定手順**:
1. レポート内の「Recommendation候補 (AI選定待ち)」セクションの候補10件を確認
2. 必要に応じて候補JSONファイル（`*_recommendation-candidates.json`）も参照
3. 5件程度を選定し、各記事に選定理由（1行）を記載
4. セクションを以下のフォーマットに書き換える
...
レポート生成後、Recommendation選定後に以下を自動検証:

- [ ] 全必須セクションが存在する（MUST READ / SHOULD READ / OPTIONAL / SKIP / Recommendation）
- [ ] Recommendationセクションに「AI選定待ち」が残っていない
- [ ] 記事数の合計がFeedlyの未読件数と一致する
- [ ] 前日のレポートが存在する場合、構造（セクション順序・テーブル列）が一致する

**検証失敗時**: 不足セクションを補完してからユーザーに提示する。
...
レポート再生成時、過去の重複履歴をチェックするが**当日の日付は必ずフィルタから除外**すること。当日を含めると自分自身のレポートの記事がすべて除外されるバグが発生する。

### 動的閾値

スコアリングの閾値は固定値ではなく、**高優先度（MUST READ）記事が全体の約20%になるよう動的に調整**する。記事数が少ない日は閾値を下げ、多い日は上げる。
