...
# e-Gov 公文書 XML → PDF 変換ツール (v2.0.0)

e-Gov 電子申請から返送される公文書 ZIP（XML + XSL）を、**真のレスポンシブレイアウト**で 1 ページに収まる PDF に変換します。
...
| パイプライン | extractor (lxml) → Jinja2 → CSS Grid → WeasyPrint |
...
| `--debug-dir DIR` | 無し | 中間 HTML / CSS をダンプ |
